...
type ExtensionHooks struct {
	...
	// Used to implement features like hiding the editor pane.
	ShouldSkipEditor func(m *Model) bool

	// CustomLayoutAdjust allows modifying editor dimensions before layout is applied.
	// Return (width, height, true) to override, or (0, 0, false) to use default.
	CustomLayoutAdjust func(m *Model, defaultWidth, defaultHeight int) (width, height int, override bool)
